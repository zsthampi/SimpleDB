...
public class LogMgr implements Iterable<BasicLogRecord> {
	...
	public LogMgr(String logfile) {
		...
		if (logsize == 0)
			...
			currentpos = getLastRecordPosition() + TWICE_INT_SIZE;
		}
	}
	...
	public synchronized int append(Object[] rec) {
		int recsize = TWICE_INT_SIZE; // 4 + 4 bytes for the integer that points
									// to the
									// previous log record and the next log
									// record
		for (Object obj : rec)
			recsize += size(obj);
		...
		finalizeRecord((recsize - TWICE_INT_SIZE));
		return currentLSN();
	}
	...
	private int getFirstRecordPosition() {
		...
		return mypage.getInt(LAST_POS + INT_SIZE);
	}
